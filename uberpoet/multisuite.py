...
from . import commandlineutil, cpprojectgen, blazeprojectgen
from .loccalc import count_loc
from .cpulogger import CPULogger, apply_cpu_to_traces
...
from uberpoet.util import (
    check_dependent_commands,
    grab_mac_marketing_name,
    makedir,
    sudo_enabled,
)
...
class CommandLineMultisuite(object):
    ...
    def parse_config(args):
        ...
        (
            parser.add_argument(
                "--bazel_command",
                default="bazel",
                help="The path to the Bazel binary.  Defaults to `bazel`.",
            ),
        )
        (
            parser.add_argument(
                "--pod_command",
                default="pod",
                help="The path to the CocoaPods binary.  Defaults to `pod`.",
            ),
        )

        commandlineutil.IosAppGenerationConfig.add_app_gen_options(parser)
        ...
        commandlineutil.IosAppGenerationConfig.validate_app_gen_options(out)

        return out

    # noinspection PyAttributeOutsideInit
    def config_to_vars(self, config):
        self.app_gen_options = commandlineutil.IosAppGenerationConfig()
    ...
    def make_context(self, log_dir, output_dir, test_build):
        ...
        self.mock_pods_app_project = join(
            self.mock_output_dir, "Pods", "Pods.xcodeproj"
        )
        ...
        has_dot = (
            self.app_gen_options.dot_file_path
            and self.app_gen_options.dot_root_node_name
        )
        if test_build:
            ...
            if not has_dot:
                logging.warning(
                    "Removing dot mock app type due to lack of dot file to read. "
    ...
    def build_app_type(self, gen_type, wmo_enabled):
        ...
        self.project_generator.gen_app(
            app_node,
            node_list,
            self.app_gen_options.swift_lines_of_code,
            self.app_gen_options.objc_lines_of_code,
            self.app_gen_options.loc_json_file_path,
        )

        swift_loc = count_loc(self.mock_output_dir)
        ...
        if self.run_xcodebuild:
            ...
            if self.project_generator_type == "cocoapods":
                subprocess.check_call(
                    [self.pod_binary, "install"], cwd=self.mock_output_dir
                )
            ...
            with open(build_log_path, "w") as build_log_file:
                if self.project_generator_type == "buck":
                    subprocess.check_call(
                        [self.buck_binary, "build", "//..."], cwd=self.mock_output_dir
                    )
    ...
    def verify_dependencies(self):
        ...
        if missing == xcode:
            logging.error(
                "Xcode command line tools do not seem to be installed / are missing in your path."
            )
        elif missing == [self.buck_binary]:
            logging.error(
                "Specified buck command not available.  Did you install it in your path?"
            )
        elif missing == [self.bazel_binary]:
            logging.error(
                "Specified bazel command not available.  Did you install it in your path?"
            )
        elif missing == [self.pod_binary]:
            logging.error(
                "Specified pod command not available.  Did you install it in your path?"
            )
    ...
    def multisuite_setup(self):
        ...
        if (
            self.project_generator_type == "buck"
            or self.project_generator_type == "bazel"
        ):
            ...
            self.project_generator = cpprojectgen.CocoaPodsProjectGenerator(
                self.mock_output_dir
            )
        else:
            raise ValueError(
                "Unknown project generator type: " + str(self.project_generator_type)
            )
    ...
    def run_multisuite(self):
        ...
        if self.project_generator_type == "buck":
            make_custom_buckconfig_local(self.buckconfig_path)
        ...
        if self.trace_cpu:
            ...
            apply_cpu_to_traces(self.build_trace_path, self.cpu_logger, start_time)
