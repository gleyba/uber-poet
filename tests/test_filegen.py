...
from uberpoet.filegen import (
    ...
    FuncType,
    Language,
    get_func_call_template,
    get_import_func_calls,
...
class TestFileGen(unittest.TestCase):
    def test_func_call_template(self):
        ...
        self.assertEqual(
            get_func_call_template(Language.SWIFT, Language.SWIFT, FuncType.SWIFT_ONLY),
            swift_to_swift_func_call_template,
        )
        self.assertEqual(
            get_func_call_template(Language.SWIFT, Language.OBJC, FuncType.SWIFT_ONLY),
            swift_to_objc_func_call_template,
        )
        self.assertEqual(
            get_func_call_template(
                Language.SWIFT, Language.SWIFT, FuncType.OBJC_FRIENDLY
            ),
            swift_to_swift_objc_friendly_func_call_template,
        )
        self.assertEqual(
            get_func_call_template(
                Language.SWIFT, Language.OBJC, FuncType.OBJC_FRIENDLY
            ),
            swift_to_objc_friendly_func_call_template,
        )
        ...
        self.assertEqual(
            get_func_call_template(
                Language.OBJC, Language.SWIFT, FuncType.OBJC_FRIENDLY
            ),
            objc_to_swift_func_call_template,
        )
        self.assertEqual(
            get_func_call_template(
                Language.OBJC, Language.OBJC, FuncType.OBJC_FRIENDLY
            ),
            objc_to_objc_func_call_template,
        )
...
MyClass0().complexStuff0(arg: "4")"""
        ...
        self.assertEqual(
            get_import_func_calls(Language.SWIFT, [swift_import]),
            expected_swift_func_calls,
        )
...
[[[MyClass_0 alloc] init] complexCrap2:4 stuff:@"2"];"""
        ...
        self.assertEqual(
            get_import_func_calls(Language.OBJC, [objc_import]),
            expected_objc_func_calls,
        )
    ...
    def _create_import(self, name, language):
        ...
        if language == Language.SWIFT:
            class_functions = {
                FuncType.SWIFT_ONLY: [0, 1, 2],
                FuncType.OBJC_FRIENDLY: [0],
            }
        elif language == Language.OBJC:
            class_functions = {FuncType.OBJC_FRIENDLY: [0, 1, 2]}
        file_result = FileResult(file_name, "body", [], {0: class_functions}, language)
        return {
