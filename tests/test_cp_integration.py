...
from uberpoet.commandlineutil import GenProjCommandLine
...
class TestCocoaPodsIntegration(unittest.TestCase):
    def verify_genproj(
        self, app_path, dir_file_count, swift_file_count, objc_file_count
    ):
    ...
    def test_dot_genproj_with_loc_mappings(self):
        ...
        loc_test_fixture_path = os.path.join(
    ...
    def test_flat_multisuite_mocking_calls(self):
        test_cloc_out_path = os.path.join(
            os.path.dirname(__file__), "fixtures", "cloc_out.json"
        )
